...
TEST(PriorityList, Simple) {
    ...
    auto rr = p.getNextAbility(s, t, Second(0.0), Second(0.0));
    ASSERT_EQ(rr, gunslinger_smugglers_luck);
    s->setAbilityCooldown(gunslinger_smugglers_luck, Second(60));
    rr = p.getNextAbility(s, t, Second(0.1), Second(0.1));
    ASSERT_EQ(rr, gunslinger_hunker_down);
    s->setAbilityCooldown(gunslinger_hunker_down, Second(35.1));
    rr = p.getNextAbility(s, t, Second(0.2), Second(0.2));
    ASSERT_EQ(rr, dirty_fighting_dirty_blast);
    rr = p.getNextAbility(s, t, Second(1.7), Second(1.7));
    ASSERT_EQ(rr, dirty_fighting_dirty_blast);
    rr = p.getNextAbility(s, t, Second(35), Second(35));
    ASSERT_EQ(rr, dirty_fighting_dirty_blast);
    rr = p.getNextAbility(s, t, Second(35.1), Second(35.1));
    ASSERT_EQ(rr, gunslinger_hunker_down);
    s->setAbilityCooldown(gunslinger_hunker_down, Second(70.2));
    rr = p.getNextAbility(s, t, Second(60), Second(60.0));
    ASSERT_EQ(rr, gunslinger_smugglers_luck);
}
} // namespace
