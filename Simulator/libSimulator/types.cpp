...
FinalStats getFinalStats(const RawStats &rawStats, const StatChanges &statChanges) {
    ...
    CriticalRating cr =
        (rawStats.criticalRating + statChanges.criticalRatingBonus) ;
    double critChance = 0.05 + detail::getCriticalChance(m) + detail::getCriticalChance(cr) + statChanges.flatCriticalBonus;
    FinalStats ret;
    ...
    double bonusDamage = (detail::getBonusDamage(m) + detail::getBonusDamage(p)) * (1.0+statChanges.bonusDamageMultiplier);
