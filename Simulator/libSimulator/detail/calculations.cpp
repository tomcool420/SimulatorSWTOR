...
[[nodiscard]] double getFTBonusDamage(FTPower ftbonus) { return ftbonus.getValue() * 0.23; }
[[nodiscard]] double getBonusDamage(Mastery mastery) { return mastery.getValue() * 0.20001; }
[[nodiscard]] double getBonusDamage(Power power) { return power.getValue() * 0.23; }
[[nodiscard]] double getDamageReduction(Armor armor) {
    return (armor.getValue() / (armor.getValue() + 349.0 * 75 + 800));
}
[[nodiscard]] Second getReducedDuration(Second time, double alacrity) { return time / (1.0 + alacrity); }
