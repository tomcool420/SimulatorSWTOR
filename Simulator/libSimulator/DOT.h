...
class DOT : public Debuff {
  public:
    ...
    DOT(AbilityId iid, AbilityInfo info):
            Debuff(iid),_DefaultTickRate(info.time),_ability(iid,info){
...
public:
    ...
    OnAbilityHitDot(AbilityId iid, AbilityInfo info, T && call) :
       DOT( iid,  info),_lambda(std::forward<T>(call)){}
