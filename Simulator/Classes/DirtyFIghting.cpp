...
class BloodyMayhem : public Debuff {
  public:
    BloodyMayhem() : Debuff(dirty_fighting_bloody_mayhem){
    ...
    switch(id){
        case dirty_fighting_dirty_blast:{
            ...
            if(getExploitedWeakness()){
                ...
                abl->addOnHitAction(std::make_shared<ConditionalApplyDebuff>(std::make_unique<DOT>(dirty_fighting_exploited_weakness,ewInfo)));
            }
            return abl;
        }
